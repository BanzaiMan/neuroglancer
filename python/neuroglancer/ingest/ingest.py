...
import downsample_scales
from tqdm import tqdm

from tasks import Task, TaskQueue
from base import  BUCKET_NAME, QUEUE_NAME, PROJECT_NAME, Storage
import chunks
from volumes import HDF5Volume, DVIDVolume, Pinky

PROCESSING_CHUNK = (2048, 2048, 64)
...
class Runner(object):
    ...
    def upload_info(self):
        ...
        #     filename='info',
        #     content=json.dumps(dict(info))
        # )
        # self._storage.flush('')

    def upload_chunks(self):
        xyzranges = ( range(0, vs, cs) for vs, cs in zip(self._volume.shape, self._chunk_size) )
        i = 0
        tasks = []
